...
L0_no_Q <- function() {
  expand_grid(
    ...
    mutate(prob = P_w(world) * interpret(message, world, inter)) %>% 
    group_by(message, inter) %>% 
    mutate(prob = prob / sum(prob)) %>% 
    ungroup() %>% 
    arrange(message, inter)
...
U1_no_Q <- function() {
  L0_no_Q() %>% 
    group_by(world, message) %>% 
    summarise(no_Q_util = sum(P_i(inter) * (log(prob) - cost(message)))) %>% 
    ungroup()
}

U1 <- function() {
  # U1_no_Q_res <- U1_no_Q() %>% 
  #   rename(no_Q_util = util)
  U1_with_Q() %>%  
    left_join(U1_no_Q(), by = c("world", "message")) %>% 
    mutate(util = k * no_Q_util + util) %>% 
    ungroup()
  # U1_with_Q() %>% 
  #   rowwise() %>% 
  #   mutate(util = {
